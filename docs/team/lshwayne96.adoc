...
** _What it does:_ This feature displays a list of modules which the student is recommended to read based on existing GradTrak modules and graduation requirements specific to the student's course of study.
...
** _Justification:_ This feature significantly benefits students who are unfamiliar with the exact graduation requirements of their course, or are unsure of what modules to read for future semesters.
...
will likely serve as a prerequisite for higher level modules. This allows students to plan their modules systematically so that they are on the right track towards graduation.
...
check each module for its prerequisites in the student's module list, and determine whether it contributes to the student's course requirements and if so, the requirement type of highest priority that it satisfies.
In particular, it was necessary to liaise with the implementation of the graduation-checking feature to ensure that the recommended modules actually bring the student closer to graduation.


* *Minor enhancement*: added additional parameters for the module finding feature, allowing the student to search GradTrak modules by
